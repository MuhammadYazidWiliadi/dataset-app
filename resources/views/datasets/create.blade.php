        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">Tambah Dataset Baru</h4>
                </div>
                    ...
                    <form action="{{ route('datasets.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('id_topik') is-invalid @enderror" id="id_topik" name="id_topik" required>
                                ...
                                @foreach($topiks as $topik)
                                    <option value="{{ $topik->id }}" {{ old('id_topik') == $topik->id ? 'selected' : '' }}>
                                        {{ $topik->topik }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_dataset') is-invalid @enderror" 
                                   id="nama_dataset" name="nama_dataset" value="{{ old('nama_dataset') }}" required>
                            @error('nama_dataset')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('metadata_info') is-invalid @enderror" 
                                      id="metadata_info" name="metadata_info" rows="3">{{ old('metadata_info') }}</textarea>
                            @error('metadata_info')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
